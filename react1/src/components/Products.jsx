...
const Products = () => {
  ...
  return (
    <div style={{backgroundColor:rendomColor}} className={`flex text-white  items-center gap-2 flex-col `}>
      ...
      <button onClick={handleBGchange} className="mt-10 p-3 rounded-full bg-[#ff00f7] font-bold">rendom background color generator</button>
      </div>
        ...
        products.map((item, index) => {
          return (
            <>
              ...
              >
                <h1>{item.title}</h1>
                <span> {(selected == item.id || selectedItem.indexOf(item.id) !== -1) ? "△" : "▽"}</span>
              </div>
              {mutiSelect
                ? selectedItem.indexOf(item.id) !== -1 && (
                    <p className="bg-gray-500 w-80 rounded-3xl p-5 mt-0">
                      {item.description}
                    </p>
                  )
                : selected == item.id && (
                    <p className="bg-gray-500 w-80 rounded-3xl p-5 m-5">
                      {item.description}
                    </p>
                  )}
            </>
